...
public class RandomGroupController {
    ...
    private func loadFromPersistentStore(){
        let fetchRequest: NSFetchRequest = Group.fetchRequest()
        do {
            let groups = try CoreDataStack.context.fetch(fetchRequest)
            self.groups = groups
        } catch let error {
            NSLog("Error fetching groups from storage: \(error.localizedDescription)")
        }
    }
